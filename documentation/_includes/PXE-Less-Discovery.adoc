...
This cusomtom partition template utilizes host.facts only to decide what the install device should be.  First it examines some possible host manufacturer and productname identifiers to determine a specific make:model of a laptop using a nvme ssd (ie: 'nvme0n1').  Second, using simlar logic it looks for virtual machines on the RHV platform using virtio disks (id: 'vda').  Lastly, it defaults to using 'sda'.
